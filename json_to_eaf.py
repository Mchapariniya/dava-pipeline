...
def create_eaf_structure(media_file: Optional[str] = None) -> ET.Element:
    ...
    for lt_id, lt_attrs in [
        ...
        ("sentiment-lt", {"TIME_ALIGNABLE": "true", "GRAPHIC_REFERENCES": "false"}),
    ]:
...
def convert_json_to_eaf(json_path: str, output_eaf_path: str, 
    ...
    # 7. Sentiment tier (if available in segments) -- written by analysis/sentiment.py
    sentiment_ann = None
    has_sentiment = any(seg.get("sentiment") for seg in segments)
    if has_sentiment:
        sentiment_ann = create_tier(root, "Sentiment", "sentiment-lt")

    # Whether the transcript already carries exact per-segment topic labels
    # (written by analysis/topic_modeling.py). If so we use them directly for a
    # precise Topics tier instead of the fuzzy CSV keyword matching below.
    has_seg_topics = any(seg.get("topic_label") for seg in segments)
    if has_seg_topics and topics_ann is None:
        topics_ann = create_tier(root, "Topics", "topic-lt")
    ...
    for seg_idx, segment in enumerate(segments):
        ...
        # Preferred path: the segment already has an exact topic_label (from
        # analysis/topic_modeling.py) -> one precise annotation per segment.
        # Fallback path: fuzzy keyword matching against the topics CSV.
        if topics_ann is not None:
            seg_topic = segment.get("topic_label")
            if seg_topic and seg_topic != "misc":
                score = segment.get("topic_score")
                topic_text_out = (f"{seg_topic} ({score:.2f})"
                                  if isinstance(score, (int, float)) else seg_topic)
                try:
                    ann_id = f"a{annotation_counter}"
                    add_annotation(topics_ann, ann_id, start_ms, end_ms, topic_text_out, time_order)
                    annotation_counter += 1
                except Exception as e:
                    print(f"  WARNING: Failed to add topic annotation: {e}")
            elif topics_map and len(topics_map) > 0:
                # Fallback: simplified keyword matching against representative docs
        ...
        # Add sentiment annotation (from analysis/sentiment.py)
        if sentiment_ann is not None:
            sentiment = segment.get("sentiment")
            if sentiment:
                s_score = segment.get("sentiment_score")
                s_text = (f"{sentiment} ({s_score:+.2f})"
                          if isinstance(s_score, (int, float)) else sentiment)
                try:
                    ann_id = f"a{annotation_counter}"
                    add_annotation(sentiment_ann, ann_id, start_ms, end_ms, s_text, time_order)
                    annotation_counter += 1
                except Exception as e:
                    print(f"  WARNING: Failed to add sentiment annotation: {e}")
